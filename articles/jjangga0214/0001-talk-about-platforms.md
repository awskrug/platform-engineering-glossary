...
자, *BigCo* 는 {{ Self-Service }}(Self-Service)의 필요성을 인식했습니다. 그러나 이미 확립된 레거시(Legacy) 인프라와 지금껏 일해 오며 익숙해진 사고 방식으로 인해서 어떻게 해야 좋을지 판단하는 데 어려움을 겪었죠. 중앙 집중식 자동화 도구에 대한 투자는 이미 있었기 때문에, 첫 번째 노력은 애플리케이션 딜리버리(Delivery) 팀이 인프라를 셀프 프로비저닝(Self-Provisioning) 할 수 있는 {{ Self-Service }} 기능을 만드는 것이었습니다.
...
우리는 이러한 유형의 접근 방식을 '{{ Superficial Private Cloud }}'(Superficial Private Cloud)라고 부릅니다. 즉, 중앙 집중식 제어의 양을 줄이려는 실제 의도 없이 매우 제한된 방식으로 딜리버리(Delivery) 팀이 사용할 수 있도록 하는 것입니다. 기존에 사용하던 가상화 플랫폼을 크게 바꾸지 않고 포장만 그럴듯하게 하는 것이라 할 수 있죠.
...
제가 만난 대부분의 조직에는 '재사용을 위한 구축'(build for re-use)이라는 기본 철학이 있었습니다. 즉, 미래를 염두에 두고 기능들을 중앙에 집중화하여 만들어 두면, 나중에 재사용으로 비용을 절감하면서도 이미 검증된 기능으로 위험을 회피할 수 있다는 것입니다.
...
이번엔 이 곳을 편의상 *WebBiz* 라고 명칭해볼게요. 아주 큰 조직은 아니었지만 딱 *BigCo* 와 동일한 종류의 과제에 직면할 만큼의 규모는 있던 곳이었습니다. 인프라, 애플리케이션 및 데이터 측면에서도 적지 않은 레거시(Legacy)를 보유하고 있었죠. 그러나 이미 말했듯 너무 거대한 회사는 아니었기에 비교적 빠르게 개선해 볼 수 있을 만큼은 작았습니다.
